    ...
    return (
        <nav className="bg-white sticky top-0 z-50 border-b border-app-border">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 gap-4">
                {/* Logo */}
                <Link to="/" className="flex item-center gap-2 text-[22px] font-medium shrink-0">
                ...
                <div className="w-full flex items-center justify-end gap-4 lg:gap-10">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button className="relative p-2 rounded-xl" onClick={() => setIsCartOpen(true)}>
                            ...
                            {cartCount > 0 && <span className="absolute -top-1 -right-1 size-4 bg-app-orange text-white text-[10px] rounded-full">{cartCount}</span>}
                        </button>
                        {/* User */}
                        <div className="relative">
                            {user ? (
                                ...
                                <div className="flex-center gap-2">
                                    <Link to='/login' className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-950 rounded-full hover:bg-green-950-light transition-colors"><UserIcon size={16} /> Sign In</Link>
                            ...
                            {userMenuOpen && (
                                ...
                                <div className="fixed inset-0 z-40" onClick={()=>setUserMenuOpen(false)}/>
                                    <div className="absolute right-0 mt-2.5 w-56 bg-white rounded-xl shadow-lg border border-app-border py-2 z-50 animate-fade-in">
                                        {user && (
                                            <div className="px-4 py-2 border-b brder-app-border">
                                        ...
                                        <div onClick={()=>setUserMenuOpen(false)}>
                                            ...
                                            {user?.isAdmin && (
                                                <Link to='/admin/products' className="dropdown-link" ><ShieldIcon className="text-app-orange-dark" size={26} /> <span className="text-app-orange-dark">Admin Panel</span></Link>
                                            )}
                                            {user && (
